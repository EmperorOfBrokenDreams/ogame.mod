...
func RunScript(bot *ogame.OGame, ogb *ogb.Ogb, ctx context.Context, scriptName, code string) error {
	...
	// Technology
	e.Define("ENERGYTECHNOLOGY", ogame.EnergyTechnologyID)
	e.Define("LASERTECHNOLOGY", ogame.LaserTechnologyID)
	e.Define("IONTECHNOLOGY", ogame.IonTechnologyID)
	e.Define("HYPERSPACETECHNOLOGY", ogame.HyperspaceTechnologyID)
	e.Define("PLASMATECHNOLOGY", ogame.PlasmaTechnologyID)
	e.Define("COMBUSTIONDRIVE", ogame.CombustionDriveID)
	e.Define("IMPULSDRIVE", ogame.ImpulseDriveID)
	e.Define("HYPERSPACEDRIVE", ogame.HyperspaceDriveID)
	e.Define("ESPIONAGETECHNOLOGY", ogame.EspionageTechnologyID)
	e.Define("COMPUTERTECHNOLOGY", ogame.ComputerTechnologyID)
	e.Define("ASTROPHYSICS", ogame.AstrophysicsID)
	e.Define("INTERGALACTICRESEARCHNETWORK", ogame.IntergalacticResearchNetworkID)
	e.Define("GRAVITATIONTECHNOLOGY", ogame.GravitonTechnologyID)
	e.Define("WEAPONTECHNOLOGY", ogame.WeaponsTechnologyID)
	e.Define("SHIELDINGTECHNOLOGY", ogame.ShieldingTechnologyID)
	e.Define("ARMOURTECHNOLOGY", ogame.ArmourTechnologyID)
	...
	e.Define("ExecIn", func(t string, callback func()) {
		...
		if err != nil {
			...
			d = time.Duration(secs * int64(time.Second))
		}
		<-time.After(d)
		callback()
	})

	e.Define("Build", bot.Build)
