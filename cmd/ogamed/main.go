...
import (
	...
	"github.com/0xE232FE/ogame.mod/cmd/ogamed/database"
	"github.com/0xE232FE/ogame.mod/cmd/ogamed/ogb"
	"github.com/0xE232FE/ogame.mod/cmd/ogamed/skv"
	"github.com/0xE232FE/ogame.mod/cmd/ogamed/webserver"
...
func start(c *cli.Context) error {
	...
	//params.CaptchaCallback = captchaSolverCallback
	...
	account, _, err := bot.FindAccount(universe, language, playerid, accounts, servers)
	if err != nil {
		log.Println(err)
	}
	...
	bot.RegisterHTMLInterceptor(func(method string, urlString string, params url.Values, payload url.Values, pageHTML []byte) {
		...
		cl := bot.GetClient()
		database.Traffic.RPS = int64(cl.GetRPS())
		...
		if method == "POST" {
			...
			ownCelestials := bot.GetCachedCelestials()
			var ownCoords []ogame.Coordinate
			for _, celestial := range ownCelestials {
				ownCoords = append(ownCoords, celestial.GetCoordinate())
			}
			attacks, err = e.ExtractAttacks(pageHTML, ownCoords)
			if err == nil {
	...
	e.GET("/bot/ip", ogame.GetIPHandler)
	...
	*/
	...
	e.GET("/building-queue", GetBuildingQueueHandler)
	e.GET("/cache", webserver.GetCachedDataHandler)
...
func captchaSolverCallback(question, icons []byte) (int64, error) {
	...
	fnLoadImage := func(data []byte) (image.Image, error) {
	...
	questionHashSum := md5.Sum(question)
	questionMatches := []QMatcher{} // key: Name Value: Hash

	var myClient = &http.Client{Timeout: 10 * time.Second}
	fnGetJson := func(url string, target interface{}) error {
	...
	fnGetJson("https://www.dropbox.com/s/udxs4v8bnfhm74i/QuestionMatch.json?dl=1", &questionMatches)

	log.Printf("Loaded %d matches.", len(questionMatches))
	log.Printf("Try to find Question with md5 Hash %x", questionHashSum)

	qMatch := QMatcher{}
	for _, q := range questionMatches {
		if q.QuestionHash == fmt.Sprintf("%x", questionHashSum) {
			// Question Found
			qMatch = q
			log.Println("Question Found", q)
			break
		}
	}

	fnCropImg := func(img image.Image, crop image.Rectangle) (image.Image, error) {
	...
	iconsToCrop, _ := fnLoadImage(icons)
	for i := 0; i < 4; i++ {
		iconBuffer := bytes.NewBuffer([]byte{})
		img, err := fnCropImg(iconsToCrop, image.Rect((i*60), 0, (i+1)*60, 60))
		if err != nil {
			log.Println(err)
			return 0, err
		}
		png.Encode(iconBuffer, img)
		if qMatch.IconHash == fmt.Sprintf("%x", md5.Sum(iconBuffer.Bytes())) {
			log.Printf("Answer is: %d", i)
			return int64(i), nil
		}
	}
	return 0, errors.New("No solution for captcha found.")
}

// CrynogarSolver solve Captchas like a Ninja
func CrynogarSolver(apiKey string) ogame.CaptchaCallback {
	return func(question, icons []byte) (int64, error) {
		...
		req, _ := http.NewRequest(http.MethodPost, "http://localhost:8888/solve", body)
...
// CrynogarSolver2 solve Captchas like a Ninja
func CrynogarSolver2(solverURL string) ogame.CaptchaCallback {
	return func(question, icons []byte) (int64, error) {
		body := &bytes.Buffer{}
		writer := multipart.NewWriter(body)
		part, _ := writer.CreateFormFile("question", "question.png")
